...
public class MainVillainAI : MonoBehaviour {
	...
	void Update ()
	{
		...
		if (range >= 17) {
			...
			//Debug.Log ("stop");
			anim.SetBool ("SlashEffect", false);
			transform.animation.CrossFade (meleeAttack.name);
			rigidbody2D.velocity = new Vector2 (attackSpeed, rigidbody2D.velocity.y);
			
		} else if (range <= 10) {
			//Debug.Log ("Knockback ready, waiting for collison.");
			if (knockbackCount <= 0) {
